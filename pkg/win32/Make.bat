...
echo.
echo Building %APPNAME% Installer for %CONFIGURATION% configuration...

"%WIX%\bin\candle" -nologo -dLIBSSH2DIR="%LIBSSH2DIR%" -dWXDIR="%WXWIN%" -dPLATFORM_TOOLSET_VERSION=%2 -dVC_TOOLS_REDIST_INSTALL_DIR=%3 -dPGDIR="%PGDIR%" -dBUILDTREE="%BUILDTREE%" -dBRANDED=%BRANDED% -dBRANDINGDIR="%BRANDINGDIR%" -dAPPVENDOR="%APPVENDOR%" -dAPPNAME="%APPNAME%" -dAPPKEYWORDS="%APPKEYWORDS%" -dAPPCOMMENTS="%APPCOMMENTS%" -dAPPDESCRIPTION="%APPDESCRIPTION%" -dAPPVERSION="%1" -dCONFIGURATION="%CONFIGURATION%" -dSYSTEM32DIR="%SystemRoot%\System32" -dPFILESDIR="%ProgramFiles%" src/pgadmin3.wxs
